...
//		orderDetails.setStatusUpdatedAt(Date.valueOf(LocalDate.now()));
		OrderDetails persistentOrder = orderDetailsRepo.save(new OrderDetails(user, Date.valueOf(LocalDate.now()),"ORDERED", address, checkout.isCodStatus(), Date.valueOf(LocalDate.now())));
